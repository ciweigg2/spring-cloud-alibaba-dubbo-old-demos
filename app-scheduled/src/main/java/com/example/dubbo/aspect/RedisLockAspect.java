...
import com.example.dubbo.annotation.CacheKeyParser;
...
public class RedisLockAspect {
    ...
    @Pointcut("@annotation(com.example.dubbo.annotation.RedisLock)")
     ...
     * @return
     */
    private boolean getLock(ProceedingJoinPoint proceeding, int count, long currentTime) throws IllegalAccessException {
        //获取注解中的参数
     ...
     * @param proceeding
     */
    private void delLock(ProceedingJoinPoint proceeding) throws IllegalAccessException {
        Map<String, Object> annotationArgs = this.getAnnotationArgs(proceeding);
        String lockPrefix = (String) annotationArgs.get(LOCK_PRE_FIX);
        String key = (String) annotationArgs.get(LOCK_KEY);
        commonRedisHelper.delete(lockPrefix, key);
     ...
     * @return
     */
    private Map<String, Object> getAnnotationArgs(ProceedingJoinPoint proceeding) throws IllegalAccessException {
        ...
        for (Method method : methods) {
            if (method.getName().equals(methodName)) {
                ...
                String timeOut = CacheKeyParser.parse(proceeding ,redisLock.timeOut());
                result.put(TIME_OUT, redisLock.timeUnit().toSeconds(Long.valueOf(timeOut)));
                return result;
            }
        }
        return null;
